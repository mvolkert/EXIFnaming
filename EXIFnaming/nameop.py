...
def rename_HDR(mode: str = "", folder: str = r"HDR\w*"):
    """
    ...
    for (dirpath, dirnames, filenames) in os.walk(inpath):
        ...
        mode_calc = mode if mode else os.path.basename(dirpath)
        for filename in filenames:
            if mode_calc in filename: continue
            match = re.search(matchreg, filename)
            if match:
                _rename_match(dirpath, filename, mode_calc, match)
            else:
                log().info("no match: %s", filename)
        for dirname in dirnames:
            match = re.search(matchreg, dirname)
            if match:
                _rename_match(dirpath, dirname, mode_calc, match)
